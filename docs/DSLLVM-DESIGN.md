...
- Default CPU: `meteorlake` (Intel Core Ultra 7 165H SoC).
- Default features (grouped as `+dsmil-optimal`):

  - AVX2, AVX-VNNI (CPU)
  - AES, VAES, SHA, GFNI (CPU, iGPU)
  - BMI1/2, POPCNT, FMA (CPU)
  - MOVDIRI, WAITPKG (CPU)

This centralizes the "optimal flags" that would otherwise be replicated in `CFLAGS/LDFLAGS`. For detailed hardware capabilities, refer to [HARDWARE_AI_CAPABILITIES_REFERENCE.md](HARDWARE_AI_CAPABILITIES_REFERENCE.md).
...
DSMIL defines **9 layers (3–9) and 104 devices**, with 48 AI devices and **~1440 TOPS INT8** across all layers. These capabilities are distributed across Intel Meteor Lake's NPU (30 TOPS), iGPU (40 TOPS), CPU AMX (32 TOPS), and AVX-512 (~10 TOPS).
...
* For **per-function cost decisions** (inlining, unrolling, vectorization choice), need <1ms inference.
...
For a detailed phased implementation guide and reading paths, refer to [AI_ARCHITECTURE_PLANNING_GUIDE.md](AI_ARCHITECTURE_PLANNING_GUIDE.md) and [ESSENTIAL_PLANNING_DOCUMENTS.md](ESSENTIAL_PLANNING_DOCUMENTS.md).
...
* **System Memory**: 64GB DDR5 SODIMM 5600 (Total, supports up to 128GB DDR5 SODIMM 5600 ECC) - NPU (2-4GB), iGPU (4-8GB) reserved.
...
| v1.3 | 2025-11-28 | SWORDIntel/DSMIL Team | Updated Repo Layout, Version, Status, and enhanced consistency with build guide. |
