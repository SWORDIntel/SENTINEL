...
```bash
# Set DSLLVM_ROOT to the absolute path of your DSLLVM clone
# Example: export DSLLVM_ROOT="/path/to/your/DSMIL/DSLLVM"
# For this guide's location (in DSMIL/):
export DSLLVM_ROOT="$(realpath "$(dirname "$0")")/DSLLVM"
export PATH=$DSLLVM_ROOT/build/bin:$PATH
export CC="$DSLLVM_ROOT/build/bin/dsmil-clang"
export CXX="$DSLLVM_ROOT/build/bin/dsmil-clang++"
...
```bash
cmake -S . -B build \
  -DCMAKE_C_COMPILER="${DSLLVM_ROOT}/build/bin/dsmil-clang" \
  -DCMAKE_CXX_COMPILER="${DSLLVM_ROOT}/build/bin/dsmil-clang++" \
...
A `.gitignore` file has been configured in the DSLLVM repository to exclude common build artifacts such as object files (`*.o`), static/shared libraries (`*.a`, `*.so`, `*.dylib`, `*.dll`), and various executables (`*.exe`, `*.bin`, `*.out`). This ensures that only source code and essential project files are tracked by Git.

If you encounter issues with previously tracked build files, ensure your Git index is updated by running:
```bash
git rm -r --cached .
git add .
```
...
```bash
# Configure
# Create a dedicated build directory. Using a RAM disk (e.g., /dev/shm) is recommended for speed.
# Example: BUILD_DIR="/dev/shm/dsmil_llvm_build"
# Or: BUILD_DIR="build"
...
- **Hardware Optimization**: Intel Meteor Lake (NPU + GPU + CPU) targeting. For detailed hardware capabilities and selection, refer to [HARDWARE_AI_CAPABILITIES_REFERENCE.md](HARDWARE_AI_CAPABILITIES_REFERENCE.md).
...
# Set this to the absolute path of your DSLLVM clone
export DSLLVM_ROOT="/path/to/your/DSLLVM/clone" # e.g., "$(realpath "$(dirname "$0")")/DSLLVM"
export PATH=$DSLLVM_ROOT/build/bin:$PATH
...
- **Location**: `/path/to/LAT5150DRVMIL/` (if available)
